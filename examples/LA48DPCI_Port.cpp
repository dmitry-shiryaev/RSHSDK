 ...
 * 
 * \file LA48DPCI_Port.cpp
 * \date 24.12.2015
 * 
 * \~english
 * \brief
 * Digital port IO for LA-48DPCI device
 * 
 * \~russian
 * \brief
 * Работа с цифровым портом для устройства ЛА-48DPCI
 * 
 */
...
int main()
{
	...
	RshDllInterfaceKey DevKey(BoardName, (void**)&device); 
	...
	{
		cout << "There are no configuration ports (bpi.confs is empty)!" << endl;
		return SayGoodBye(Client, RSH_API_SUCCESS);
	}
	...
	//Сначала нужно настроить конфигурационный порт

	//Тип операции - запись данных в конфигурационный порт.
	p.operationType = RshInitPort::Write; 	
	// Адрес конфигурационного регистра для порта XP1.
	p.portAddress =  bpi.confs[0].address;
	// Установить все биты на вывод для порта XP1.(данные с платы)
	p.portValue = 0x80; 
	...
	std::wcout << "0x" << std::hex << p.portValue << L" was successfully written to " << bpi.confs[0].name << endl;

	// Адрес порта XP1_B.
	p.portAddress = bpi.ports[1].address; 
	// Числовое значение для записи в порт XP1_B.
	p.portValue = 0xff;

	// Вывод данных в цифровой порт XP1_B.
	st = device->Init(&p);	
	if (st != RSH_API_SUCCESS)
		return SayGoodBye(Client, st, "device->Init(&p);");

	std::wcout << "0x" << std::hex << p.portValue << L" was successfully written to " << bpi.ports[1].name << endl;
	...
	//Перенастройка конфигурационного порта для работы цифровых портов "на вход"

	// Операция записи в конфигурационный порт.
	p.operationType = RshInitPort::Write; 
	// Адрес конфигурационного регистра для порта XP2.
	p.portAddress   = bpi.confs[1].address; 
	 // Установить все биты на ввод для порта XP2.(данные на плату)
	p.portValue    = 0x9b;
	...
	std::wcout << "0x" << std::hex << p.portValue << L" was successfully written to " << bpi.confs[1].name << endl;
	...
	// Установим адрес порта XP2_B.
	p.portAddress = bpi.ports[4].address; 

	// Занулим поле, в которое будут помещены данные с порта XP2_B.
	p.portValue = 0x0; 
	...
	std::wcout << "Read operation from " << bpi.ports[4].name << " was successfull, value=0x" << std::hex << p.portValue << endl;
